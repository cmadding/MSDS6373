...
#Data Cleaning
#remove NA's from Temperature data
HourlyWeather = HourlyWeather[which(!is.na(HourlyWeather$`HourlyDryBulbTemperature`)),]
#Remove any rejected \ change state rows
SMUSwipe = SMUSwipe[SMUSwipe[,2]=="CardAdmitted",]
#Remove duplicate time ID swipes
...
#Create a TempTime in SMUSwipe to merge Temperature data
SMUSwipe$TempTime <- format(SMUSwipe$LDT,"%Y-%m-%d %H")
#Create a TempTime in HourlyWeather to merge Temperature data
HourlyWeather$TempTime <- format(HourlyWeather$DATE,"%Y-%m-%d %H")
#Left join on TempTime to pull in hourly weather data
SMUSwipe=left_join(SMUSwipe, HourlyWeather, 'TempTime')
...
```{r}
#forcast hour
foreHour_diff=fore.arma.wge((tail(x,length(x)/72)), phi = est$phi, theta = est$theta, lastn = F, n.ahead = 48, limits=F)
```
