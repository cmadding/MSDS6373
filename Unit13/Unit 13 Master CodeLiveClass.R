...
#Again This is purely an academic exercise. Temperature and Particles are assumed to be known when they would not be in practrice. 
# These could be forecast and then used as inputs.  
...
##### MLP MODEL FOR CARDIAC MORTALITY DATA ... if you knew the temp and particles
...
#temp
fit.mlp.temp = mlp(ts(CMsmall$temp,frequency = 52),reps = 20, comb = "median")
plot(fit.mlp.temp)
fore.mlp.temp = forecast(fit.mlp.temp, h = 52)
plot(fore.mlp.temp)

#particles
fit.mlp.part = mlp(ts(CMsmall$part,frequency = 52),reps = 20, comb = "median")
...
CMDF_fore = data.frame(Week = ts(seq(2,508,1)),temp = ts(c(CMsmall$temp,fore.mlp.temp$mean)), part = ts(c(CMsmall$part,fore.mlp.part$mean)),temp_1 = ts(c(CMsmall$temp_1,fore.mlp.temp1)))
CMDF_fore


#forecast cmort using mlp with forecasted xreg (don't need to forecast week.)
CMsmallDF = data.frame(Week = ts(CMsmall$Week),temp = ts(CMsmall$temp), part = ts(CMsmall$part), temp_1 = ts(CMsmall$temp_1))
fit.mlp = mlp(ts(CMsmall$cmort, frequency = 52),reps = 20,comb = "median",xreg = CMsmallDF)
fit.mlp
plot(fit.mlp)
#CMDF = data.frame(Week = ts(CM$Week),temp = ts(CM$temp), part = ts(CM$part), temp_1 = ts(CM$temp_1))
